...
def train():
    ...
    data_name = os.path.basename(data_args.data_path.rstrip("/"))
    ...
    # Save model to "weights" subdirectory
    if training_args.save_model:
        weight_output_dir = os.path.join(training_args.output_dir, "weights", data_name)
        os.makedirs(weight_output_dir, exist_ok=True)
        trainer.save_state()
        safe_save_model_for_hf_trainer(trainer=trainer, output_dir=weight_output_dir)
