...
for TF in os.listdir('../Dataset/ChIP-seq'):
    model_path = os.path.join('../Main/ModelWeight/multiModel', TF)
    if os.path.isdir(model_path):
        print(f"✅ already exist, skip: {TF}")
        continue
    ...
    optimizer = optim.AdamW(optimizer_grouped_parameters, lr=1e-4, eps=1e-8,
                            betas=(0.9, 0.999))
    ...
    for Epoch in range(MaxEpoch):
        # train
        NeuralNetwork.train()
        TrainProgressBar = tqdm(TrainLoader, colour='blue')
        ...
        # valid
        NeuralNetwork.eval()
        ValidProgressBar = tqdm(TestLoader, colour='green')
        ...
        if Performance_token[5] > aupr_token:
            ...
            os.makedirs('ModelWeight/multiModel/' + TFsName, exist_ok=True)
